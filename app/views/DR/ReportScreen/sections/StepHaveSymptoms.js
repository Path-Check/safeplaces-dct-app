...
const StepHaveSymptoms = ({ setCompleted, sendData }) => {
  ...
  const [symptoms, setSymptoms] = useState(haveSymptoms == true ? `${sustantive} al menos uno de estos síntomas` : haveSymptoms == false ? `No ${sustantive.toLowerCase()} ninguno de estos síntomas` : '')
  if (symptoms) {
    setCompleted(true);
    sendData(symptoms);
  }
  ...
  const setSelectedOption = selected => {
    ...
    selected = selected === `${sustantive} al menos uno de estos síntomas` ? true: false
    setGlobalState({
      type: 'ADD_ANSWERS',
